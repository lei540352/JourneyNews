package com.journey.news.upimg;
...
public class MainActivity111 extends AppCompatActivity {
    ...
    private int mType;
    private MyThread timeThread;//倒计时
    private ImgThread imgThread;//加载图片
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button addnext =  findViewById(R.id.addnext);
        ...
        addnext.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new Thread(new ImageThread(mActivitiesList)).start();
            }
        });
    }
    ...
    private void initData(){
        ...
        resultListBean2.setProcess(100);
        ...
        imgThread = new ImgThread(mActivitiesList);
    ...
    Handler handler = new Handler(){
        public void handleMessage(Message msg){
            switch (msg.what){
                ...
                //    mRecommendActivitiesAdapter.notifyDataSetChanged();
                    //优化刷新adapter的方法
                    mRecommendActivitiesAdapter.notifyDataImg();
                    break;
                case 3:
                    int id = msg.arg1;
                    //刷新适配器
                //    mRecommendActivitiesAdapter.notifyDataSetChanged();
                    //优化刷新adapter的方法
                    mRecommendActivitiesAdapter.notifyDataImgK(id);
                    break;
            }
    ...
    class ImgThread implements Runnable {
        //用来停止线程
        boolean endImgThread;
        List<RecommendActivitiesBean.ResultListBean> mRecommendActivitiesList;

        public ImgThread(List<RecommendActivitiesBean.ResultListBean> mRecommendActivitiesList){
            this.mRecommendActivitiesList = mRecommendActivitiesList;
        }
            ...
            while(!endImgThread){
                try{
                    ...
                    for(int i = 0;i < mRecommendActivitiesList.size();i++){
                        //拿到每件商品的时间差，转化为具体的多少天多少小时多少分多少秒
                        //并保存在商品time这个属性内
                        int process = mRecommendActivitiesList.get(i).getProcess();
                        //如果时间差大于1秒钟，将每件商品的时间差减去一秒钟，
                        // 并保存在每件商品的counttime属性内
                        if(process < 100) {
                            endImgThread = false;
                            mRecommendActivitiesList.get(i).setProcess(process+1);
                        }else{
                            endImgThread = true;
                        }
                    }
    ...
    class ImageThread implements Runnable {
        ...
        public ImageThread(List<RecommendActivitiesBean.ResultListBean> mRecommendActivitiesList){
            this.mRecommendActivitiesList = mRecommendActivitiesList;
        }
        ...
        public void run() {
            ...
            while(!endImgThread){
                try{
                    Thread.sleep(10);
                    if(id < mRecommendActivitiesList.size()){
                        int process = mRecommendActivitiesList.get(id).getProcess();
                        //如果时间差大于1秒钟，将每件商品的时间差减去一秒钟，
                        // 并保存在每件商品的counttime属性内
                        if(process < 100) {
                            mRecommendActivitiesList.get(id).setProcess(process+5);
                        }else{
                            id++;
                        }
